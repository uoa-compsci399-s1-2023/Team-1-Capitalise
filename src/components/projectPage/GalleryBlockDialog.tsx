...
import { Dialog, DialogContent, DialogTitle, Grid, DialogActions, useTheme, Button, TextField, Box, Stack, OutlinedInput } from '@mui/material'
  ...
  return (
    ...
    >
        ...
        >
          {imgUrls.map((url, index) => (
            <ImgThumbnail
              key={index}
              url={url}
              {...{ blockIndex, tabIndex, imgUrls, setImgUrls }}
              imgIndex={index}
            />
          ))}
        </Grid>
      ...
      <DialogContent>
        ...
        {/* Can only upload 4 files at a time */}
        {newImgFiles.length < 4 &&
          <Button
            variant='outlined'
